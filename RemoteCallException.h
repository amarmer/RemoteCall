...
namespace RemoteCall
{
    ...
    {
    public:
        enum ErrorType {NoError, TransportError, ServerError, InvalidFunction, InvalidInterface, InvalidClass, InvalidClassInstance, InvalidMethod};
        ...
        void AddCallLocation(const CallLocation& callLocation)
        {
            (std::exception&)*this = std::exception((std::string(callLocation) + " " + what()).c_str());
        }
